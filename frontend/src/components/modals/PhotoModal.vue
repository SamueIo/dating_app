<template>
  <!-- Overlay container -->
  <div
    class="fixed inset-0 z-[9990] bg-black/70 backdrop-blur-sm flex items-center justify-center"
    @click="close"
  >

    <!-- Close button -->
    <button
      class="absolute z-[9999] top-4 right-4 text-black bg-white/80 
    ...
    <button
      ...
      class="absolute right-3 top-1/2 -translate-y-1/2 z-[9999] text-white text-5xl font-bold rounded-full select-none shadow-lg"
    >
      ›
    </button>
 ...
 * 
 * Props:
 * - images: Array of image objects with `url` or `file_name` properties
 * - startIndex: Initial image index to display (default 0)
 * 
 * Emits:
 * - close: emitted when the user closes the lightbox
 * 
 * Features:
